...
public class JoinMessageCommand {
    ...
    public static final LiteralCommandNode<CommandSourceStack> COMMAND_NODE = Commands.literal("joinmessage")
            .requires(source -> source.getSender() instanceof Player && source.getSender().hasPermission("vorplexcore.customjoinmessages"))
            .then(Commands.literal("set")
                    .then(Commands.argument("message", StringArgumentType.greedyString())
                            .executes(JoinMessageCommand::setSelf)
                    )
                    .executes(JoinMessageCommand::help)
            )
    ...
    private static int help(final CommandContext<CommandSourceStack> ctx) {
        ...
        player.sendRichMessage("<light_purple>|<st>      </st><dark_purple>[</dark_purple><white>Custom Join Messages Tips</white><dark_purple>]</dark_purple><st>     </st>|</light_purple>");
        player.sendRichMessage("<white>- To set your custom join message type /joinmessage set <message></white>");
        player.sendRichMessage("<white>- To clear your custom join message type /joinmessage clear</white>");
    ...
    private static int setSelf(final CommandContext<CommandSourceStack> ctx) {
        ...
        if (strippedMessage.length() > plugin.getConfig().getInt("JoinMessages.CustomJoinMessages.maxlength", 128)) {
            ...
            try {
                ...
                player.sendMessage(plugin.getBasicMM().deserialize(plugin.getCustomJoinMessagesCache().get(player.getUniqueId()), Placeholder.parsed("prefix", prefix), Placeholder.component("name", Component.text(player.getName()))));
                return Command.SINGLE_SUCCESS;
    ...
    private static int clearSelf(final CommandContext<CommandSourceStack> ctx) {
        final Player player = (Player) ctx.getSource().getSender();
        try {
            plugin.getStorageProvider().deleteJoinMessage(player.getUniqueId());
    ...
    private static void logException(Exception e, String playerName, UUID playerUUID) {
        plugin.getComponentLogger().error("An Error was encountered while trying to delete a custom join message for: {} (UUID: {})", playerName, playerUUID == null ? "null" : playerUUID);
