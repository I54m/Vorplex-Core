    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (strings[0].equalsIgnoreCase("set")) {
            ...
            if (ChatColor.stripColor(ChatColor.translateAlternateColorCodes('&', joinmessage.toString())).replace(" ", "").length() > plugin.getConfig().getInt("JoinMessages.customjoinmessages.maxlength")) {
                ...
                try {
                    String sql = "SELECT * FROM `vectorcore_joinmessages` WHERE UUID='" + player.getUniqueId().toString() + "';";
                    ...
                    if (results.next()) {
                        String sql1 = "UPDATE `vectorcore_joinmessages` SET `RawMessage`='" + joinMessageRaw + "' WHERE UUID='" + player.getUniqueId().toString() + "';";
                        PreparedStatement stmt1 = plugin.connection.prepareStatement(sql1);
                        stmt1.executeUpdate();
                        stmt1.close();
                    } else {
                        String sql1 = "INSERT INTO `vectorcore_joinmessages` (`UUID`, `RawMessage`)" +
            ...
            if (!player.hasPermission("vectorcore.customjoinmessages.admin")){
            ...
            try{
                String sql1 = "DELETE FROM `vectorcore_joinmessages` WHERE UUID='" + targetUUID.toString() + "'";
                PreparedStatement stmt1 = plugin.connection.prepareStatement(sql1);
                stmt1.executeUpdate();
                stmt1.close();
