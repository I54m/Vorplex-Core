    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (strings[0].equalsIgnoreCase("set")) {
            ...
            if (ChatColor.stripColor(ChatColor.translateAlternateColorCodes('&', leavemessage.toString())).replace(" ", "").length() > plugin.getConfig().getInt("LeaveMessages.customleavemessages.maxlength")) {
                ...
                try {
                    String sql = "SELECT * FROM `vectorcore_leavemessages` WHERE UUID='" + player.getUniqueId().toString() + "';";
                    ...
                    if (results.next()) {
                        String sql1 = "UPDATE `vectorcore_leavemessages` SET `RawMessage`='" + leaveMessageRaw + "' WHERE UUID='" + player.getUniqueId().toString() + "';";
                        PreparedStatement stmt1 = plugin.connection.prepareStatement(sql1);
                        stmt1.executeUpdate();
                        stmt1.close();
                    } else {
                        String sql1 = "INSERT INTO `vectorcore_leavemessages` (`UUID`, `RawMessage`)" +
            ...
            if (!player.hasPermission("vectorcore.customleavemessages.admin")){
            ...
            try{
                String sql1 = "DELETE FROM `vectorcore_leavemessages` WHERE UUID='" + targetUUID.toString() + "'";
                PreparedStatement stmt1 = plugin.connection.prepareStatement(sql1);
                stmt1.executeUpdate();
                stmt1.close();
