...
public class CommandPreProcess implements Listener {
    ...
    public static List<String> servers = plugin.getConfig().getStringList("VectorServer.serverslist");
    ...
    public void onCommandPreProcess(PlayerCommandPreprocessEvent event){
        ...
        if (args.length >= 2){
            ...
            if (!player.hasPermission("vectorcore.server.switch")) return;
            for (String server : servers){
                if (server.equalsIgnoreCase(args[1]) || server.contains(args[1])){
                    player.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("VectorServer.sendingtoservermessage").replace("%server%", server)));
